            ...
            <table border="0" width="100%" style=" border-spacing: 0;margin:0;padding:0;margin-top:25px; ">
                ...
                <?php
                    if($_POST){
                        ...
                        $sqlmain = "SELECT appointment.appoid, appointment.appodate, appointment.status,
                            time_slots.slot_id, time_slots.start_time, time_slots.end_time,
                            staff.stuname, departments.depname as department,
                            resident.resname, resident.restel,
                            services.name as service_name, services.category,
                            (SELECT COUNT(*) + 1 FROM appointment a2 
                             WHERE a2.appodate = appointment.appodate 
                             AND a2.scheduleid = appointment.scheduleid 
                             AND a2.created_at <= appointment.created_at) as appointment_number
                            FROM appointment 
                            INNER JOIN time_slots ON appointment.scheduleid = time_slots.slot_id
                            INNER JOIN staff_service ON staff_service.staff_id = appointment.staff_id
                            INNER JOIN staff ON appointment.staff_id = staff.stuid
                            INNER JOIN departments ON staff.stucialties = departments.id
                            INNER JOIN resident ON appointment.resid = resident.resid
                        ...
                        $sqlmain .= " ORDER BY appointment.appodate DESC, time_slots.start_time";
                    } else {
                        $sqlmain = "SELECT appointment.appoid, appointment.appodate, appointment.status,
                            time_slots.slot_id, time_slots.start_time, time_slots.end_time,
                            staff.stuname, departments.depname as department,
                            ...
                            INNER JOIN services ON appointment.service_id = services.service_id
                            WHERE appointment.status != 'cancelled'
                            ORDER BY appointment.appodate DESC, time_slots.start_time";
                    }
                   ...
                   <td colspan="4">
                       <center>
                        ...
                        <tr>
                                ...
                                <th class="table-headin">Resident</th>
                                    ...
                                    for ($x=0; $x<$result->num_rows; $x++){
                                        ...
                                        echo '<tr>
                                            ...
                                            <td>
                                                ...
                                                .'</td>
                                            <td>
                                                '.$resname.'<br><span style="font-size:12px;color:var(--primarycolor);">'.$restel.'</span>'
                                                .'</td>
                                            <td>
                                            ...
                                            <td style="text-align:center;">'.
                                            ($status == 'active' ? '<span style="color:var(--success);">Active</span>' : 
                                            ($status == 'completed' ? '<span style="color:var(--primarycolor);">Completed</span>' : 
